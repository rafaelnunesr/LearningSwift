...
piano.play(music, usingPedals: true)
...
let acousticGuitar = AcousticGuitar(brand: "Roland", stringGauge: "heavy")
acousticGuitar.tune()
acousticGuitar.play(music)


class Amplifier {
    private var _volume: Int // only accessible inside the class, underscore at the beginning ot the name emphasizes that's a private implementation (convention)
    private(set) var isOn: Bool // it can be read by outside users, but not written to
...
class ElectricGuitar: Guitar {
    let amplifier: Amplifier // has a relationship, not is a relationship
    ...
    override func play(_ music: Music) -> String {
        let preparedNotes = super.play(music)
        return "Play solo \(preparedNotes) at the volume \(amplifier.volume)"
    }
}
